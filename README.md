...
- **Суммаризация документов** - автоматическое создание краткого содержания загруженных документов
- **📚 Реферативный перевод** - подробный анализ документов с генерацией PDF
- **🌍 Автоматический кросс-языковой поиск** - работа с документами на любых языках без настройки
- **🌐 Веб-поиск** - поиск информации в интернете с суммаризацией (⚠️ экспериментально, возможны ограничения)
...
- [**Суммаризация документов**](docs/SUMMARIZATION.md) - автоматическое создание краткого содержания
- [**Реферативный перевод**](docs/REFERAT.md) - подробный аналитический обзор с PDF
- [**Веб-поиск**](docs/WEB_SEARCH.md) - поиск информации в интернете ⚠️ экспериментально
- [**Автоматический кросс-языковой поиск**](docs/AUTO_MULTILINGUAL.md) - работа с документами на разных языках
- [**Настройка размера контекста**](docs/CONTEXT_TUNING.md) - оптимизация количества фрагментов для ответов
...
5. **Суммаризация документа**: выберите документ из списка и введите команду `summary` для получения краткого содержания
6. **Реферативный перевод**: выберите документ и введите команду `referat` для создания подробного обзора с PDF
...
- `POST /api/summarize` - Получить краткое содержание документа
  ```json
  {
    "document_id": 1
  }
...
#### Embeddings и Поиск
- `EMBEDDING_MODEL` - модель для векторизации (по умолчанию: sentence-transformers/paraphrase-multilingual-mpnet-base-v2)
- `CHUNK_SIZE` - размер фрагмента текста (по умолчанию: 500)
- `CHUNK_OVERLAP` - перекрытие между фрагментами (по умолчанию: 50)
- `SEARCH_LIMIT` - количество фрагментов в контексте для генерации ответа (по умолчанию: 7)
  - Увеличьте для более полных ответов (например, 10-15)
  - Уменьшите для более быстрых ответов (например, 3-5)
- `MIN_SIMILARITY` - минимальный порог схожести для фильтрации результатов, 0.0-1.0 (по умолчанию: 0.4)
  - Увеличьте для более строгой фильтрации (например, 0.5-0.6)
  - Уменьшите для более мягкой фильтрации (например, 0.3)
